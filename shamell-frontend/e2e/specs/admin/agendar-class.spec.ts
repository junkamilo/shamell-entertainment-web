...
test.describe("Agendar — class tab", () => {
  ...
  test("opens BOOK PRIVATE CLASS panel without event catalog fetches", async ({
    page,
  }) => {
    ...
    for (const pattern of blockedPatterns) {
      await page.route(pattern, async (route) => {
        await route.fulfill({
          ...
          body: JSON.stringify({
            message: "Should not be called in class-only mode",
          }),
        });
      });
    }
  ...
  test("switches between BOOK and BOOK PRIVATE CLASS tabs", async ({
    page,
  }) => {
