...
- **Fixed ticket packages (FIXED EVENT):** `purchaseMode` stays `fixed_ticket`; public detail adds `ticketMode: SINGLE | PACKAGES` + `packages[]` / `activities[]`. Admin: `/admin/on-coming-events` event form (`features/admin/events`) — checkbox **Sell ticket packages**, activities + packages API under `/api/v1/upcoming-events/admin/events/:eventId/{activities,fixed-packages}`. Checkout body accepts optional `packageId`. Backend notes: `shamell-backend/src/modules/upcoming-events/README.md`.
