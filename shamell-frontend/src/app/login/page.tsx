...
export default function LoginPage() {
  ...
  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        setError("Invalid credentials or authentication flow not configured yet.");
        return;
      }
    ...
    <main className="bg-background min-h-screen pt-20 pb-16 px-4">
      <section className="max-w-2xl mx-auto border border-gold/30 bg-background/80 p-8 md:p-10">
        <div className="flex flex-col items-center text-center mb-8">
          ...
          <h1 className="font-brand text-gold text-2xl md:text-4xl tracking-[0.16em] mb-2">LOGIN</h1>
          <p className="font-script text-gold-light text-2xl">Secure access to your account</p>
        </div>
        ...
        <div className="mt-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 text-xs tracking-wide">
