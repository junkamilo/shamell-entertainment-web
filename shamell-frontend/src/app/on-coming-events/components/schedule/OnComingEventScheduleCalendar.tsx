...
export function OnComingEventScheduleCalendar({ model }: Props) {
  ...
  return (
    <div className="flex flex-col" aria-label="Event calendar">
      <div className="mb-4 flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className="rounded-lg border border-gold/25 p-1.5 text-gold/80 transition hover:border-gold/50 hover:bg-gold/10"
            aria-label="Previous month"
          >
            <ChevronLeft className="h-4 w-4" />
          </button>
          <span className="min-w-[9rem] text-center font-body text-sm text-foreground/90">
      ...
      {/* Week strip */}
      <div className="mb-5 grid grid-cols-7 gap-1.5" role="list" aria-label="Days of week">
        {headers.map((label, idx) => {
          ...
          return (
            ...
            >
              <span className="text-[10px] font-medium tracking-wide text-foreground/50">{label}</span>
              <span
                className={cn(
      ...
      {/* Month grid */}
      <div className="grid grid-cols-7 gap-1" role="grid" aria-label={`${monthLabel(viewYear, viewMonth)}`}>
        {headers.map((h) => (
          <div
            key={`h-${h}`}
            className="pb-1 text-center text-[10px] font-medium text-foreground/45"
            role="columnheader"
          >
            {h}
          </div>
        ...
        {grid.cells.map((cell, i) => {
          ...
          return (
            ...
            >
              ...
              {cell.isEventDay ? (
                <Star
                  className="absolute -right-0.5 -top-0.5 h-3 w-3 fill-gold text-gold"
                  aria-hidden
                />
              ) : null}
            </motion.button>
      ...
      {tooltipText ? (
        <motion.p
          ...
          animate={{ opacity: 1, y: 0 }}
          className="mt-4 rounded-lg border border-gold/20 bg-gold/5 px-3 py-2 text-center text-xs text-foreground/85"
          role="status"
        >
          {tooltipText}
        </motion.p>
      ...
      {model.mode === "FIXED_EVENT" ? (
        <div className="mt-4 flex flex-wrap justify-center gap-4 text-[10px] text-foreground/55">
