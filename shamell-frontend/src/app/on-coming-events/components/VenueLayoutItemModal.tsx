...
function formatEventDisplay(label: string | null, eventDateIso: string | null): string {
  ...
  try {
    return new Date(eventDateIso).toLocaleString("en-US", {
      dateStyle: "long",
      timeStyle: "short",
    });
  } catch {
  ...
  return (
    ...
    >
      <button
        ...
        onClick={onClose}
      />
      <div className="relative z-10 max-h-[min(92dvh,44rem)] w-full max-w-md overflow-y-auto overscroll-contain rounded-t-2xl border border-gold/25 bg-[linear-gradient(180deg,#1a1018,#0f0a12)] p-4 pb-[max(1.25rem,env(safe-area-inset-bottom,0px))] shadow-2xl sm:max-h-[calc(100dvh-2rem)] sm:rounded-2xl sm:p-6 sm:pb-[max(1.5rem,env(safe-area-inset-bottom,0px))]">
        <h2 id="venue-item-modal-title" className="font-display text-xl text-gold sm:text-2xl">
          {title}
        </h2>
          ...
          >
            <label className="block text-xs text-foreground/70">
              ...
              <input
                ...
                className="mt-1 w-full rounded-lg border border-gold/20 bg-black/30 px-3 py-2.5 text-sm"
                autoComplete="name"
              />
            </label>
            <label className="block text-xs text-foreground/70">
            ...
            <button
              ...
              className="w-full rounded-lg bg-gold py-3 text-xs font-semibold uppercase tracking-wider text-black hover:bg-gold-light disabled:opacity-60"
            >
