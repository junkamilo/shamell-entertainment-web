  ...
  return (
    <aside className="shrink-0 border-b border-shamell-line-soft px-3 py-2 sm:px-4 sm:py-3">
      ...
      {visibleTiles.length === 0 ? (
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 [-ms-overflow-style:none] [scrollbar-width:none] md:grid md:grid-cols-3 md:overflow-visible md:pb-0 lg:grid-cols-6 [&::-webkit-scrollbar]:hidden">
          {visibleTiles.map((tile) => (
            <div key={tile.id} className="shrink-0 md:shrink">
              <PaletteTile {...tile} />
            </div>
          ))}
        </div>
