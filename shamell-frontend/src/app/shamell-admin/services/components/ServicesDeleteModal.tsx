...
export default function ServicesDeleteModal({ isOpen, title, isDeleting, onClose, onConfirm }: Props) {
  return (
    <AdminModal title="Delete service" isOpen={isOpen} onClose={onClose}>
      <div className="space-y-5 font-body text-sm text-foreground/85">
        <p>
          Permanently delete <span className="font-brand text-gold">{title}</span>? This will remove it from the
          catalog and cloud storage (cannot be undone).
        </p>
