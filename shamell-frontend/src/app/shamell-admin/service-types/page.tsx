...
export default function ShamellAdminServiceTypesPage() {
  ...
  const loadTypes = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`${apiBaseUrl}/api/v1/services/types/admin`, {
      ...
      if (!response.ok) {
        toast({
          variant: "destructive",
          title: "Error",
          description: parseErrorMessage(data, "No se pudieron cargar los tipos de servicio."),
        });
        return;
      }
      setTypes(Array.isArray(data) ? (data as ServiceTypeItem[]) : []);
    } catch {
      toast({
        variant: "destructive",
        title: "Sin conexión",
  ...
  const originalName = editingId
    ? (types.find((item) => item.id === editingId)?.name.trim() ?? "")
    : "";
  ...
  return (
    <div className="mx-auto w-full max-w-6xl">
      <AdminModuleHero title="Tipos De Servicio" actionLabel="Nuevo Tipo" onAction={onHeroAction} bordered={false} />
      ...
      <section className="mt-8 rounded-md bg-black/20 p-6 md:p-7">
        ...
        {!isLoading && filteredTypes.length === 0 ? (
          <p className="mt-4 text-sm text-foreground/70">Aún no hay tipos de servicio.</p>
        ) : null}
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {filteredTypes.map((item) => (
            <article
              key={item.id}
              className="overflow-hidden rounded-2xl border border-gold/20 bg-black/25 shadow-[0_6px_24px_rgba(0,0,0,0.35)]"
            >
              ...
              <div className="p-4">
                <p className="font-brand text-3xl leading-none text-gold/90">
