...
type Props = {
  pendingDelete: ServiceTypeItem | null;
  isDeleting: boolean;
  onClose: () => void;
  onConfirm: () => void;
};

export default function ServiceTypesDeleteModal({
  pendingDelete,
  isDeleting,
  onClose,
  onConfirm,
  ...
  return (
    <AdminModal title="Delete service type" isOpen={Boolean(pendingDelete)} onClose={onClose}>
      <div className="space-y-5 font-body text-sm text-foreground/85">
        <p>
          Permanently delete{" "}
          <span className="font-brand text-gold">{pendingDelete?.name}</span>? You will not be able
          to recover it.
        </p>
