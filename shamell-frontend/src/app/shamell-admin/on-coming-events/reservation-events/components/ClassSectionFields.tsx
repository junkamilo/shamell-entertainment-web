    ...
    <div className="rounded border border-white/10 p-3">
      ...
      <div className="grid gap-2 sm:grid-cols-2">
        <label className="block sm:col-span-2">
          <span className={`${fieldLabelClass} block`}>Label (optional)</span>
          <input
            value={section.label}
            disabled={disabled}
        ...
        <label className="block">
          ...
          <input
            ...
            onChange={(e) =>
              onPatch({
                defaultCapacity: Number.parseInt(e.target.value, 10) || 20,
              })
            }
            className="mt-1 w-full rounded border border-gold/20 bg-black/30 px-2 py-1.5 text-sm"
          />
        ...
        <label className="block">
          <span className={pairedFieldLabelClass}>
            Section price override (USD, optional)
          </span>
          <input
            type="number"
            min={0}
            step={0.01}
            disabled={disabled}
            placeholder="Uses event base price"
          ...
          <span className="mt-1 block text-[10px] text-foreground/50">
            Leave empty to use the event base price.
          </span>
        </label>
      </div>
    </div>
