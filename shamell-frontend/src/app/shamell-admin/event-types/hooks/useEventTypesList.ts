...
import type { EventTypeItem, EventTypesStats, FilterTab } from "../types/eventTypes.types";
...
export function useEventTypesList() {
  ...
  const loadTypes = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const items = await fetchAdminEventTypes(token);
      setTypes(items);
    } catch (err) {
  ...
  const stats: EventTypesStats = useMemo(() => {
