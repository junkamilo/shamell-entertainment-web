...
type AgendaHubPageContentProps = {
  peticionesBadge: number;
};

export default function AgendaHubPageContent({ peticionesBadge }: AgendaHubPageContentProps) {
  return (
    <div className="mx-auto w-full max-w-5xl">
      ...
      <div className="mt-10 grid gap-4 md:grid-cols-3">
        {cards.map(({ href, title, subtitle, icon: Icon, fire, showBadge }) => (
          <Link
            key={href}
            href={href}
            className={`group shamell-glass-card relative ${fire ? "shamell-glass-card--fire" : ""}`}
          >
            {showBadge && peticionesBadge > 0 ? (
