...
import type { VenueLayoutPromo } from "@/hooks/use-venue-layout-settings";

export async function fetchVenueLayoutSettings(): Promise<VenueLayoutPromo | null> {
  const base = getPublicApiBaseUrl();
  const response = await fetch(`${base}/api/v1/venue-layout/settings`, { cache: "no-store" });
