...
import { SERVICE_TYPE_CODES } from "@/lib/contacto/contactInquiryConstants";
import {
...
export default function ContactInquiryPhaseServiceType(props: ContactInquiryPhaseProps) {
  ...
  const { data, setData, setStepError, serviceTypeOptions, setDetailModal } = props;
  return (
    <div className="space-y-4">
      <p className="font-body text-base leading-relaxed text-foreground/75 md:text-lg">
        Select all service types that apply to this inquiry (one or more). Tap the eye to read the
        full offering.
      </p>
          ...
          ? serviceTypeOptions.map((row) => {
              ...
              return (
                ...
                >
                  <label
                    htmlFor={chkId}
                    className="flex min-w-0 flex-1 cursor-pointer flex-col gap-1 p-4"
                  >
