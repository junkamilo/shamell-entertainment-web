...
export type CreateFixedEventCheckoutBody = {
  customerName: string;
...
export async function createFixedEventCheckoutSession(
  slug: string,
  body: CreateFixedEventCheckoutBody,
): Promise<{ ok: true; clientSecret: string } | { ok: false; message: string }> {
  const base = getPublicApiBaseUrl();
  const response = await fetch(
    `${base}/api/v1/upcoming-events/${encodeURIComponent(slug)}/fixed-event/checkout-session`,
    {
      method: "POST",
