...
import { formatRelativeEn, slugifyDisplay } from "./galleryCategoriesDisplay";

describe("galleryCategoriesDisplay", () => {
  beforeEach(() => {
    vi.useFakeTimers();
  ...
  it("slugifyDisplay normalizes names", () => {
    expect(slugifyDisplay("  Live Performance  ")).toBe("live-performance");
    expect(slugifyDisplay("Café & Noches")).toBe("cafe-noches");
    expect(slugifyDisplay("---")).toBe("");
  });
});
