...
export async function patchAdminReservationEventTemplate(
  token: string,
  id: string,
  body: Partial<ReservationEventTemplateBody>,
  ...
  if (!response.ok || !data || typeof data !== "object" || !("id" in data)) {
    const msg = nestApiErrorMessage(data, "Could not update reservation event.");
