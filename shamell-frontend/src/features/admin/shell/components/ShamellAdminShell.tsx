...
import {
  ...
  ShellMenuIcon,
  ShellSidebarCollapseIcon,
  ShellSidebarExpandIcon,
  ShellViewSiteIcon,
  type AdminIcon,
...
import { fetchAdminVenueReservations, VENUE_RESERVATIONS_ADMIN_PATH } from "@/features/admin/venue-reservations";
...
function breadcrumbFromPath(pathname: string): string[] {
  ...
  if (parts[0] !== "admin" && parts[0] !== "shamell-admin") return ["Admin"];
  const crumbs = ["Admin"];
  if (parts.length === 1) {
    crumbs.push("Agenda");
    return crumbs;
...
export function ShamellAdminShell({ children }: { children: React.ReactNode }) {
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const router = useRouter();
  const { permissions, role, user } = useAdminSession();
  ...
  const visibleNavEntries = useMemo(
    () => filterAdminNavEntries(adminNavEntries, permissions),
    [permissions],
  );
  const roleLabel =
    role === "SUPER_ADMIN" ? "Super admin" : role === "ADMIN" ? "Admin" : "Administrator";
  ...
  useEffect(() => {
    ...
    if (raw) {
      try {
        ...
        setAdminName(u.fullName ?? user?.fullName ?? "Administrator");
        setAdminEmail(u.email ?? user?.email ?? "");
      } catch {
        /* ignore */
      }
    }
  }, [router, user?.email, user?.fullName]);
    ...
    <>
      ...
      >
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          >
            <ShellCloseIcon className="h-4 w-4" strokeWidth={1.5} />
          </button>
          <button
            type="button"
          ...
          >
            {sidebarCollapsed ? (
              <ShellSidebarExpandIcon className="h-4 w-4" strokeWidth={1.5} />
            ) : (
              <ShellSidebarCollapseIcon className="h-4 w-4" strokeWidth={1.5} />
            )}
          </button>
        </div>
      </div>
      ...
      <div className="shrink-0 border-t border-gold/20 p-4 max-lg:pb-[max(1rem,env(safe-area-inset-bottom,0px))] [@media(max-height:900px)]:p-3">
        {!sidebarCollapsed ? (
          <div className="admin-panel mb-3 flex items-center gap-3 rounded-md px-3 py-2 [@media(max-height:900px)]:mb-2 [@media(max-height:900px)]:py-1.5">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-[10px] text-gold/70 [@media(max-height:900px)]:hidden">{roleLabel}</p>
            </div>
          </div>
        ) : (
        ...
        >
          <ShellViewSiteIcon className="h-3.5 w-3.5" strokeWidth={1.5} />
    ...
    <div className="admin-theme flex h-dvh min-h-0 overflow-hidden bg-transparent text-foreground">
      ...
      <div className="shamell-admin-bg flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
        <header className="admin-theme-main-header sticky top-0 z-30 flex shrink-0 items-center gap-3 px-4 max-lg:min-h-[calc(3.5rem+env(safe-area-inset-top,0))] max-lg:pt-[env(safe-area-inset-top,0px)] lg:h-14">
          ...
          >
            <ShellMenuIcon className="h-4 w-4" strokeWidth={1.5} />
          </button>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <ShellExternalLinkIcon className="h-3.5 w-3.5" />
              VIEW SITE
            </Link>
          </div>
        </header>
