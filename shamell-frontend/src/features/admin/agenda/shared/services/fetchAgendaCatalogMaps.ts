...
import { fetchAdminServicesRawShared } from "@/features/admin/services/hooks/useServicesQuery";
...
export async function fetchAgendaCatalogMaps({
  ...
  const requests: Promise<unknown>[] = [
    fetchAdminServicesRawShared(token),
  ...
  if (includeOccasions) {
    requests.push(
      fetch(`${base}/api/v1/events/occasions/admin`, { headers, cache: "no-store" }).then((res) =>
        res.json().catch(() => []),
      ),
    );
  }
  ...
  if (includeContactLines) {
    requests.push(
      fetch(`${base}/api/v1/events/contact-lines`, { headers, cache: "no-store" }).then((res) =>
        res.json().catch(() => []),
      ),
    );
  }

  const json = await Promise.all(requests);
