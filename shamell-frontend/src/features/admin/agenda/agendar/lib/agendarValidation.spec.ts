...
import { emptyAgendarFormValues, validAgendarFormValues } from "../tests/fixtures/formValues.fixture";
...
describe("getAgendarMobileSectionStatus", () => {
  ...
  it("marks event incomplete without an occasion", () => {
    const values = { ...validAgendarFormValues, occasionTypeId: "" };
    expect(getAgendarMobileSectionStatus(values).event).toBe(false);
  });
});
...
describe("validateAgendarForm", () => {
  ...
  it("requires event type", () => {
    const result = validateAgendarForm({ ...validAgendarFormValues, eventTypeId: "" });
    expect(result.error).toBe("Required field missing: Event type.");
  });
  ...
  it("rejects short location", () => {
    const result = validateAgendarForm({ ...validAgendarFormValues, location: "AB" });
    expect(result.error).toBe("Location must be between 3 and 120 characters.");
  });
});
