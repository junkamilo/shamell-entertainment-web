...
export async function fetchQuoteCheckoutClientSecret(
  ...
  try {
    ...
    const response = await fetch(
      `${base}/api/v1/bookings/public/quote/checkout?token=${encodeURIComponent(token)}`,
      { cache: "no-store" },
    );
    if (!response.ok) {
...
export async function fetchQuotePaymentSessionStatus(
  ...
  try {
    const base = getPublicApiBaseUrl();
    const response = await fetch(
      `${base}/api/v1/bookings/public/quote/session-status?session_id=${encodeURIComponent(sessionId)}`,
      { cache: "no-store" },
    );
