...
import { MODAL_LAYERS } from "../Modal/modalLayers";

export type BlockedActionModalProps = {
  isOpen: boolean;
  onClose: () => void;
  title: string;
  description: string;
    ...
    <AnimatePresence>
      {isOpen ? (
        <motion.div
          ...
          className={`admin-theme fixed inset-0 flex items-center justify-center bg-shamell-night/82 px-4 py-6 backdrop-blur-sm ${MODAL_LAYERS.overlay}`}
          role="presentation"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
