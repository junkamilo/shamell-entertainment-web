...
import { ADMIN_MEDIA_PREVIEW_OVERLAY_Z_CLASS } from "@/components/admin/adminModalLayers";
import { cn } from "@/lib/utils";

export type AdminMediaPreviewModalProps = {
  isOpen: boolean;
  onClose: () => void;
  src: string;
  title?: string;
 ...
 */
export default function AdminMediaPreviewModal({
  isOpen,
  onClose,
  src,
  title = "Preview",
      ...
      {isOpen && src ? (
        <motion.div
          ...
          className={cn(
            ...
            ADMIN_MEDIA_PREVIEW_OVERLAY_Z_CLASS,
          )}
          role="presentation"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
