...
export type MediaUploadIconVariant = "plus" | "cloud-upload";

export type MediaUploadIconButtonProps = {
  /** Passed to the hidden file input (default: images + videos). */
  accept?: string;
  multiple?: boolean;
  /** `cloud-upload` for multi-upload zones; `plus` for single-file pick rows. */
  iconVariant?: MediaUploadIconVariant;
 ...
 * For single-file pick with filename row, use {@link MediaPickControl}.
 */
export const MediaUploadIconButton = forwardRef<
  HTMLInputElement,
  MediaUploadIconButtonProps
>(function MediaUploadIconButton(
  {
    accept = "image/*,video/*",
    multiple = false,
    iconVariant = "plus",
