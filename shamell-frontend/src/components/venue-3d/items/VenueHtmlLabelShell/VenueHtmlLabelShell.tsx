...
export type VenueHtmlLabelShellVariant = "reserved" | "number";

const VARIANT_PILL_STYLE: Record<VenueHtmlLabelShellVariant, CSSProperties> = {
  reserved: {
...
const VARIANT_TAIL: Record<VenueHtmlLabelShellVariant, { outer: string; inner: string }> = {
  reserved: { outer: "#000000", inner: "#ffffff" },
  number: { outer: "rgba(168, 132, 32, 0.9)", inner: "rgba(201, 162, 39, 0.96)" },
};

export type VenueHtmlLabelShellProps = {
  label: string;
  ariaLabel: string;
  height?: number;
  variant?: VenueHtmlLabelShellVariant;
  showTail?: boolean;
