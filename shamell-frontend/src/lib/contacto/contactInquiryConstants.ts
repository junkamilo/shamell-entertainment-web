...
export function buildServiceInquireHref(
  ...
  return `${CONTACTO_PATH}?serviceType=${code}&entry=home_service_card`;
}
...
export function buildEventLineContactHref(eventId: string): string {
  ...
  if (!isContactCatalogUuid(tid)) return `${CONTACTO_PATH}?entry=home_service_card`;
  const base = `${CONTACTO_PATH}?eventId=${encodeURIComponent(tid)}&entry=home_service_card`;
