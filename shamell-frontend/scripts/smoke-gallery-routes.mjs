/**
 * Smoke test: public gallery route must not 404 (including filter querystrings).
 * Usage: npm run build && npm run start (in another terminal), then npm run smoke:gallery
 * Override base: SMOKE_BASE_URL=http://localhost:3010 node scripts/smoke-gallery-routes.mjs
 */
...
const cases = [
  ...
  {
    name: "gallery filter=all",
    path: "/gallery?filter=all",
    expectStatus: (s) => !isNotFound(s),
  },
  {
    name: "gallery filter=fire",
    path: "/gallery?filter=fire",
    expectStatus: (s) => !isNotFound(s),
  },
];
...
console.log("\nAll gallery-route smoke tests passed.");
