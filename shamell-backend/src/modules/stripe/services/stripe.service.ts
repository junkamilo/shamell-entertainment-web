...
export class StripeService implements OnModuleInit {
  ...
  onModuleInit(): void {
    const key = this.config.get<string>(STRIPE_ENV_SECRET_KEY)?.trim() ?? '';
    const webhookSecret =
      this.config.get<string>(STRIPE_ENV_WEBHOOK_SECRET)?.trim() ?? '';
    const publishableKey =
      this.config.get<string>(STRIPE_ENV_PUBLISHABLE_KEY)?.trim() ?? '';
    const nodeEnv = (
      this.config.get<string>(STRIPE_ENV_NODE_ENV) ?? ''
    ).toLowerCase();
  ...
  get client(): InstanceType<typeof Stripe> {
    if (!this.stripeClient) {
      const secretKey = this.config.get<string>(STRIPE_ENV_SECRET_KEY);
      if (!secretKey) {
  ...
  get webhookSecret(): string {
    const secret = this.config.get<string>(STRIPE_ENV_WEBHOOK_SECRET);
    if (!secret) {
      throw new InternalServerErrorException(
  ...
  frontendUrl(): string {
    const url =
      this.config.get<string>(STRIPE_ENV_FRONTEND_URL) ??
      STRIPE_DEFAULT_FRONTEND_URL;
    return url.split(',')[0]?.trim() || STRIPE_DEFAULT_FRONTEND_URL;
  }
}
