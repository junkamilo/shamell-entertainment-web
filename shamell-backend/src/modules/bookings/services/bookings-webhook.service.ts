...
import type { StripeCheckoutSessionLite } from '../types/bookings.types';
import {
...
export class BookingsWebhookService {
  ...
  async markBookingPaymentPaid(
    ...
    const payment =
      await this.repository.findWebhookPaymentBySessionId(sessionId);
    ...
    if (payment.stage === BookingPaymentStage.DEPOSIT) {
      await this.repository.updateBooking(payment.bookingId, {
  ...
  private async markBookingPaymentExpired(sessionId: string): Promise<void> {
    const payment =
      await this.repository.findExpiredWebhookPaymentBySessionId(sessionId);
  ...
  private async sendDepositPaidEmail(
    ...
    const booking = await this.repository.findBookingAdminById(bookingId);
    if (!booking) return;
    const toEmail =
