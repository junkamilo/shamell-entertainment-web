...
import { todayISODateInTimezone } from '../../reservation-event-templates/utils/reservation-event-template.util';
import { CLASS_SESSION_GENERATION_WEEKS } from '../constants/upcoming-events.constants';
import type { RegenerateClassSessionsResult } from '../types/upcoming-events.types';
import { withTemplateRegenerationLock } from './class-session-regeneration.lock';

export type { RegenerateClassSessionsResult } from '../types/upcoming-events.types';
...
async function regenerateClassSessionsInClient(
  ...
  const endIso = addDaysIso(startIso, CLASS_SESSION_GENERATION_WEEKS * 7);
