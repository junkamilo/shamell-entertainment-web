...
import { bookingInquiryCatalogEventWhere } from '../../events/utils/booking-inquiry-catalog.util';
import type { SanitizedInquiryDetails } from '../types/booking-inquiry.types';
import { trimUuid } from './booking-inquiry-uuid.util';
...
export async function resolvePrimaryServiceIdForInquiry(
  ...
  if (eventTypeId) {
    const et = await prisma.eventType.findUnique({
      ...
      select: { contactInquiryCode: true, catalogChannel: true },
    });
    if (et?.catalogChannel === EventTypeCatalogChannel.BOOKING) {
