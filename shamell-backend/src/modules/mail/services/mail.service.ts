...
import {
  ...
  ENV_APP_PUBLIC_NAME,
  ENV_MAILERSEND_API_KEY,
  ENV_MAILERSEND_FROM_EMAIL,
  ENV_MAILERSEND_FROM_NAME,
...
export type { SendTransactionalResult, TransactionalMailPayload };
...
export class MailService implements OnModuleInit {
  ...
  resolveFromEmail(): string {
    let raw = this.config.get<string>(ENV_MAILERSEND_FROM_EMAIL) ?? '';
    raw = raw.trim();
    if (
  ...
  isConfigured(): boolean {
    const apiKey = this.config.get<string>(ENV_MAILERSEND_API_KEY)?.trim();
  ...
  private resolveFromName(): string {
    return (
      this.config.get<string>(ENV_MAILERSEND_FROM_NAME)?.trim() ??
