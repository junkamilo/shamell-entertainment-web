...
import {
  MIN_CLASS_SECTION_PRICE,
  WEEKDAY_LABELS,
} from '../constants/reservation-event-templates.constants';
import type {
  ClassSectionInput,
  TemplateForDerive,
  TemplateSummaryRow,
...
import { validateSectionsNoOverlapMessage } from '../../upcoming-events/utils/class-section-overlap.util';
...
export function validateClassSectionsForWeekdays(
  ...
  for (const wd of activeWeekdays) {
    const dayLabel = WEEKDAY_LABELS[wd] ?? `Day ${wd}`;
...
export function buildTemplateSummary(row: TemplateSummaryRow): string {
